//
//  BOBookListTableViewController.m
//  Bamboo
//
...
@interface BOBookListTableViewController () <DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
@implementation BOBookListTableViewController
...
- (void)loadMoreData {
    ...
    [query orderByDescending:@"createdAt"];
    [query whereKey:@"user" equalTo:[AVUser currentUser]];
    [query whereKey:@"done" equalTo:@(self.isDone)];
    query.limit = self.limit; // 最多返回 self.limit 条结果
    query.skip = self.skip;  // 跳过 self.skip 条结果
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (error) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self performSegueWithIdentifier:@"BookInfoPushSegue" sender:self];
}
